...
<script>
    $(document).ready(function(){
        ...
        $(".write_button").on("click",function(){
            ...
            document.freeBoardForm.curPage.value = curPge;
            document.freeBoardForm.searchTitle.value = curPge;
            document.freeBoardForm.searchContent.value = curPge;

            document.freeBoardForm.action = "/forum/writeForm";
        ...
        $("#insert").on("click",function(){
            if($("#category").val() === ""){
            	Swal.fire({
            	    ...
            	    $("#category").focus(); // 'category' 필드에 포커스를 맞춤
            	});
                return;
            }
            ...
            if($.trim($("#title").val()).length === 0){
            	Swal.fire({
            	    ...
            	    $("#title").val(""); // 제목 입력 필드를 초기화
            	    $("#title").focus(); // 제목 입력 필드에 포커스를 맞춤
            	});
                return;
            }
            ...
            if($.trim($("#content").val()).length === 0){
            	Swal.fire({
            	    ...
            	    $("#content").val(""); // 내용 입력 필드를 초기화
            	    $("#content").focus(); // 내용 입력 필드에 포커스를 맞춤
            	});
                return;
            }
            ...
            $.ajax({
            	...
            	success:function(response){
            		if (response.code === 0){
            			Swal.fire({
            			    title: '성공',
            			    text: '게시글을 성공적으로 작성하였습니다.',
            			    icon: 'success',
            			    confirmButtonText: '확인'
            			}).then(() => {
            			    location.href = "/community/freeBoard"; // 게시판 페이지로 이동
            			});
            			...
            			Swal.fire({
            			    title: '오류',
            			    text: 'DB정합성 오류로 게시글 작성에 실패하였습니다.',
            			    icon: 'error',
            			    confirmButtonText: '확인'
            			});     		
    	        	} 
            			...
            			Swal.fire({
            			    title: '오류',
            			    text: '서버 응답 오류로 게시글 작성에 실패하였습니다.',
            			    icon: 'error',
            			    confirmButtonText: '확인'
            			});
    	        	}
    ...
    function uploadImage(file) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#content").summernote("insertNode", imgTag);
            },
            error: function(error) {
            	Swal.fire({
            	    title: '오류',
            	    text: '이미지 업로드에 실패하였습니다.',
            	    icon: 'error',
            	    confirmButtonText: '확인'
            	});
    	...
    	$.ajax({
            ...
            success: function(response) {
                if (response.code === 200) {
                	...
                	Swal.fire({
                	    title: '오류',
                	    text: '서버 응답 오류로 이미지 삭제에 실패하였습니다.',
                	    icon: 'error',
                	    confirmButtonText: '확인'
                	});
                }
	                ...
	                <div data-v-e0f90538="" class="dropdown">
	                    ...
	                    <div class="category_select_wrapper">
	                        <select id="category" name = "category" class="category_select">
	                            ...
	                            <option value="CMFO">FreeBoard</option>
