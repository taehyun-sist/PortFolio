...
<script>
	...
	/*
        $("#insert").on("click",function(){
            document.styleBoardForm.action = "/style/writeProc";
            document.styleBoardForm.submit();
            alert("게시글이 등록되었습니다.");
        });
		*/
        ...
        $("#cancel").on("click",function(){
            ...
            commentItem.find(".comment_input").css("display", "block"); 
            commentItem.find(".submit_comment").css("display", "block"); 
            
            document.styleBoardForm.action = "/style/styleIndex";
            document.styleBoardForm.submit();
        });
 ...
 /*       $(".btn_cancel").on("click",function(){
            ...
            commentItem.find(".comment_input").css("display", "none"); 
            commentItem.find(".submit_comment").css("display", "none"); 
        ...
        document.getElementById('file_upload').addEventListener('change', function(event) {
            ...
            // 파일명 배열을 문자열로 변환하여 #file_name 요소에 표시
            document.getElementById('file_name').textContent = fileNames.length > 0 
                ? fileNames.join(', ') // 파일 이름들을 콤마로 구분하여 표시
                : '선택된 파일 없음';
        });
        ...
        console.log(typeof jQuery); // 출력이 "function"이면 정상
...
<%@include file="/WEB-INF/views/include/navigation.jsp" %>
   <!-- 메인 컨텐츠 -->
              <form name="styleBoardForm" id="styleBoardForm" method="post" enctype="multipart/form-data">
                ...
                <div data-v-e0f90538="" class="dropdown">
                    ...
                    <div class="category_select_wrapper">
                        <select id="category" name="category" class="category_select">  
		                   <option value="CMSY" >Style</option>
                        </select>
                    </div>
                    <div data-v-e0f90538="" class="dropdown_head">
                        <input type="text" id="title" name="title" class="title_input" placeholder="[제목을 작성할 공간]">
                    </div>
                </div>
                <div data-v-e0f90538="" class="dropdown_content">
				    <textarea id="content" name="content" class="summernote" class="content_input" placeholder="[내용이 들어갈 공간]"></textarea>
				    <div class="file_upload_wrapper">
				        <input type="file" id="file_upload" name="file_upload" class="file_upload_input" multiple/>
				        <label for="file_upload" class="file_upload_label">
				            첨부파일
				        </label>
				        <span id="file_name" class="file_name_placeholder">선택된 파일 없음 (예: jsp, png...)</span> <!-- 선택된 파일명이 여기에 표시됨 -->
				    </div>
				</div>
                <div data-v-e0f90538="" class="btn_list">
                    <a data-v-7122e1e2="" data-v-e0f90538="" id = "insert" type="button" class="btn outlinegrey medium"> 작성 </a>
                    <a data-v-7122e1e2="" data-v-e0f90538="" id = "cancel" type="button" class="btn outlinegrey medium"> 취소 </a>
